...
-- import Object.Types
-- import Object.Terminal.Types

import Game.Scene.Types


initialOsdSpriteds :: IO (Map String Sprited)
initialOsdSpriteds = do
    tree <- lookupOSDs >>= load
    return $ Map.fromList $ map inner $ leafs tree
  where
    inner :: Sprited -> (String, Sprited)
    inner s = (getName $ loadedSpritedName s, s)
