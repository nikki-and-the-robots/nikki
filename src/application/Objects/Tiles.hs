...
initChipmunk space tile@(Tile sprited pos UninitializedAnimation) = do
    ...
    return $ Tile sprited chip UninitializedAnimation
...
mkShapes :: AnchoredBox s => [s] -> ([ShapeType], Vector)
mkShapes boxes@(box : _) =
...
render ptr offset (Tile s chip animation) = do
    resetMatrix ptr
