...
import Data.Char
import Data.Foldable as Foldable (any, forM_, mapM_)
...
copy path = do
  ...
  if isFile
    then copyFile path (deploymentDir </> takeFileName path)
    else
      if isDir
        then copyDirectory path (deploymentDir </> takeFileName path)
    ...
    assertWantedDeps deps =
      forM_ deps $ \dep ->
        when (isUnwantedDep dep) $
          error
            ( printf
                "executable %s depends on unwanted library %s"
                exe
                dep
            )
