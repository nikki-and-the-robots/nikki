...
frameTimes action = case action of
    State Wait{} HLeft _ -> ("wait_left", wait)
    State Wait{} HRight _ -> ("wait_right", wait)
    State Touchdown HLeft _ -> ("touchdown_left", wait)
    State Touchdown HRight _ -> ("touchdown_right", wait)
    State Walk{} HLeft _ -> ("walk_left", walk)
    State Walk{} HRight _ -> ("walk_right", walk)
    State JumpImpulse{} HLeft _ -> ("jump_left", airborne)
    State JumpImpulse{} HRight _ -> ("jump_right", airborne)
    State Airborne{} HLeft _ -> ("jump_left", airborne)
    State Airborne{} HRight _ -> ("jump_right", airborne)
    State WallSlide{} HLeft _ -> ("wallslide_left", airborne)
    State WallSlide{} HRight _ -> ("wallslide_right", airborne)
    State SlideToGrip{} HLeft _ -> ("grip_left", singleFrame)
    State SlideToGrip{} HRight _ -> ("grip_right", singleFrame)
    State Grip HLeft _ -> ("grip_left", singleFrame)
    State Grip HRight _ -> ("grip_right", singleFrame)
    State EndGripImpulse HLeft _ -> ("grip_left", singleFrame)
    State EndGripImpulse HRight _ -> ("grip_right", singleFrame)
    State UsingTerminal _ _ -> ("terminal", singleFrame)
    State (NikkiLevelFinished Passed) HLeft _ -> ("happy_left", singleFrame)
    State (NikkiLevelFinished Passed) HRight _ -> ("happy_right", singleFrame)
    State (NikkiLevelFinished Failed) HLeft _ -> ("sad_left", singleFrame)
    State (NikkiLevelFinished Failed) HRight _ -> ("sad_right", singleFrame)
